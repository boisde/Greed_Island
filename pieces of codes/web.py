...
import types, os, re, cgi, sys, json, datetime, functools, mimetypes, threading, logging, urllib, traceback
...
class Request(object):
    ...
    def _parse_input(self):
        ...
        request_method = self._environ.get('REQUEST_METHOD', None)
        content_type = self._environ.get('CONTENT_TYPE', None)
...
class WSGIApplication(object):
    ...
    def init_log(mod_dir):
        logging_msg_format = '[%(asctime)s] [%(levelname)s] [%(module)s] %(message)s'
        ...
        from tools_lib.log import RotatingFileHandlerCustomHeader
        ...
        #     version = ""
        # version_msg = str("Rate_API Processor(BL) Version %s" % version)
        # header_msg = [version_msg]

        # for example: business_logic_api/rate/logs/
        logs_path = os.path.abspath(os.path.join(mod_dir, "logs"))

        print os.path.abspath(mod_dir)
        folders = str(mod_dir).split('/')
        print folders
        prefix = {
            "api_gateway": "AG",
            "business_logic_api": "BL",
            "core_data_api": "CD",
        }[folders[-2]]
    ...
    def get_wsgi_application(self, mod_dir, debug=False):
        self._check_not_running()
        if debug:
            self._get_dynamic.append(StaticFileRoute())
        self._running = True

        WSGIApplication.init_log(mod_dir)
        ...
        def wsgi(env, start_response):
            ...
            try:
                ...
                logging.info("[timeit] [%s] [%s][%s][%s bytes] [%s]: [%d msecs]" %
                             (env['REMOTE_ADDR'], env['REQUEST_METHOD'], env['PATH_INFO'], content_length, resp_status, (t2-t1)*1000))
...
def api(func):
    ...
    def _wrapper(*args, **kw):
        try:
            ...
            if err.__class__.__name__ in ("IntegrityError", "AttributeError", "ProgrammingError"):
                ctx.response.status = '400 SQL %s' % str(err.__class__.__name__)
                resp = json.dumps({"message": str(err.args) if err.args else "un-processable"})  # 错误返回信息
            else:
                ctx.response.status = '400 %s' % str(err.__class__.__name__)
                resp = json.dumps({"message": str(err.message) if err.message else "un-processable"})  # 错误返回信息

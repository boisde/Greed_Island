...
class TimeZone(object):
    ...
    @classmethod
    def month_range(cls, year, month, value=None, tz=pytz.utc):
        # 传入的日期是本地时区
        if value:
    ...
    def datetime_to_str(cls, d):
        return d.strftime(cls.UTC_DATE_TIME_PATTERN)
    ...
    def date_to_str(cls, d):
        return d.strftime(cls.DATE_PATTERN)
...
if __name__ == '__main__':
    ...
    print 'first in 2014-08 in utc: %s' % TimeZone.datetime_to_utc(TimeZone.naive_to_aware(datetime.datetime(2014, 8, 1)))
    print 'first in 2014-08 in utc: %s' % TimeZone.datetime_to_utc(TimeZone.naive_to_aware(datetime.datetime(2014, 9, 1)))

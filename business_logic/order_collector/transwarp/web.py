    ...
    def _wrapper(*args, **kw):
        try:
            ...
            if err.__class__.__name__ in ("IntegrityError", "AttributeError", "ProgrammingError"):
                ...
                resp = json.dumps({"message": err.msg if err.msg else "un-processable"})  # 错误返回信息
            else:
                ctx.response.status = '400 %s' % str(err.__class__.__name__)
                # 错误返回信息
                if err.message and isinstance(err.message, unicode):
                    resp = json.dumps({"message": err.message.encode('utf-8')})
                elif err.message and isinstance(err.message, str):
                    resp = json.dumps({"message": err.message})
                else:
                    resp = json.dumps({"message": "un-processable"})
            logging.exception(err.__str__().decode('utf-8'))

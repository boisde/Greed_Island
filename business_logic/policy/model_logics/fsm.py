...
from task_state import TaskStateLogic
from policy import PolicyLogic
...
class FSM(object):
    ...
    def write_db(w_task_id, state_change_time, fin_state, state_info=None):
        ...
        p = PolicyLogic.find_by('id, policy_name', 'where id=?', '', w_task_id)
        TaskStateLogic.create(**dict(policy_task_id=w_task_id, policy_name=p[0].policy_name,
                                     state_change_time=state_change_time, state=fin_state,
                                     state_info=state_info))
        PolicyLogic.update(w_task_id, **dict(state=fin_state))

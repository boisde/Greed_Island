...
from mongoengine import (EmbeddedDocument, Document, StringField, DictField, DateTimeField, ListField, IntField,
                         PointField, Q)
...
from tools_lib.sstring import safe_unicode
...
class Call(Document):
    ...
    shop_id = StringField(min_length=24, max_length=24, comment='商户id')
    shop_name = StringField(max_length=64, comment='商户名称')
    shop_tel = StringField(min_length=1, max_length=14, comment='商户注册手机号')
...
class Express(Document):
    ...
    pkg_id = StringField(default="", comment=u"批次号")
    third_party = DictField(default=None, comment='第三方运单来源: name, order_id')
    biz_model = StringField(default=None, comment='业务模式: 落地配/直送/...')
    ...
    assignee = DictField(default=None, comment='id, name, tel, m_type: 其中m_type为角色,暂时记录app-name')
    watchers = ListField(default=None, comment='所有需要关注运单的人: [{id, name, tel, m_type: 其中m_type为角色,暂时记录app-name}]')

    node = DictField(default=dict(node_n={}, node_0={}),
                     comment='node_n是送达信息;node_0是取货信息. 形如: name, tel, loc{name,addr,lat,lng}, fence...')
    ...
    meta = {
        ...
        'indexes': [
            'third_party.order_id',
            'status.status',
            'status.sub_status',
            'assignee.id',
            'watchers.id',
            'watchers.watcher_type',
            'node.node_n.tel',
            'create_time',
            'update_time'
        ]
...
class Trace(Document):
    ...
    operator = DictField(default=dict, max_length=24, comment='操作人: id, name, tel, m_type')
...
class Node(Document):
    name = StringField(min_length=1, max_length=128, comment='站点名称')
...
class Address(Document):
    ...
    default_node_0 = DictField(default=dict, comment='默认发货地址: name, tel, loc={lat,lng,address}')
    node_0 = ListField(default=list, comment='发货地址库: [{name, tel, loc={lat,lng,address}},...]')
    node_n = ListField(default=list, comment='送达地址库: [{name, tel, loc={lat,lng,address}},...]')
    ...
    meta = {
        ...
        'indexes': [
            ...
            'default_node_0.name',
            'default_node_0.tel',
            'default_node_0.loc.address',
        ]
    }


class APIRecord(Document):
    """
    通过API进入的运单原始数据
    """
    token = StringField(default="", comment=u"原始token")  # 形如"token e2f6c40ce8839d4372713611aedace4b"
    shop = DictField(default=dict, comment=u"商户信息")
    data = DynamicField(default=dict, comment=u"json数据,可为{}或者[]")
    create_time = DateTimeField(default=utc_now, comment=u"创建时间")
    number = StringField(default="", comment=u"相关运单号")

    meta = {
        'db_alias': 'aeolus_connection',
        'collection': 'api_record',
        'ordering': [
            'create_time',
        ],
        'indexes': [
